...
const Sidebar: React.FC<SidebarProps> = ({ currentUser, activePage, setActivePage, isSidebarOpen, userHasPermission }) => {
  ...
  return (
    <aside className={`bg-secondary text-text-primary transition-all duration-300 ease-in-out ${isSidebarOpen ? 'w-64' : 'w-20'} h-screen flex flex-col fixed top-0 left-0 z-40`}>
      <div className="flex items-center justify-center h-20 border-b border-border">
        ...
        {isSidebarOpen && <h1 className="text-xl font-bold">EstateFlow</h1>}
      </div>
